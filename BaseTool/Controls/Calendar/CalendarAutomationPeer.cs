...
namespace BaseTool
{
    ...
    {
        /// <summary>
        /// Gets the BaseTool.Calendar associated with the CalendarAutomationPeer.
        /// </summary>
        ...
        private Grid OwningGrid
        {
            get
            {
                ...
                if (calendar.MonthControl != null)
                {
                    ...
                    else if (calendar.DisplayMode == CalendarMode.Quarter)
                    {
                        return calendar.MonthControl.QuarterView;
                    }
                    else
                    {
        ...
        /// <summary>
        /// Returns the control type for the BaseTool.Calendar that is associated with
        ...
        /// <summary>
        /// Returns the text label of the BaseTool.Calendar that is associated with this
        ...
        protected override string GetNameCore()
        {
            ...
            {
                var labeledBy = GetLabeledByCore();
                if (labeledBy != null)
                {
        ...
        IRawElementProviderSimple IGridProvider.GetItem(int row, int column)
        {
            ...
            {
                foreach (var child in OwningGrid.Children)
                {
        ...
        string IMultipleViewProvider.GetViewName(int viewId)
        {
            ...
            {
                case 0:
                    return Resources.CalendarAutomationPeer_MonthMode;
                case 1:
                    return Resources.CalendarAutomationPeer_YearMode;    
                case 2:
                    return Resources.CalendarAutomationPeer_DecadeMode;
            }

            // REMOVE_RTM: update when Jolt 23302 is fixed
            // throw new ArgumentOutOfRangeException("viewId", BaseTool.Resources.Calendar_OnDisplayModePropertyChanged_InvalidValue);
            return String.Empty;
        }
        ...
        IRawElementProviderSimple[] ISelectionProvider.GetSelection()
        {
            ...
            if (OwningGrid != null)
            {
                if (OwningCalendar.DisplayMode == CalendarMode.Month && OwningCalendar.SelectedDates != null && OwningCalendar.SelectedDates.Count != 0)
                {
                    ...
                    Primitives.CalendarDayButton  dayButton;
                    ...
                    foreach (var child in OwningGrid.Children)
                    {
                        ...
                        if (childRow != 0)
                        {
                            dayButton = child as Primitives.CalendarDayButton;

                            if (dayButton != null && dayButton.IsSelected)
                            {
                                var peer = CreatePeerForElement(dayButton);
                ...
                {
                    // return the BaseTool.Primitives.CalendarButton  which has focus
                    Primitives.CalendarButton  calendarButton;

                    foreach (var child in OwningGrid.Children)
                    {
                        calendarButton = child as Primitives.CalendarButton; 

                        if (calendarButton != null && calendarButton.IsCalendarButtonFocused)
                        {
                            var peer = CreatePeerForElement(calendarButton);
        ...
        {
            var calendar = OwningCalendar;
            var selectedDates = calendar.SelectedDates.Count;

            if (ListenerExists(AutomationEvents.SelectionItemPatternOnElementSelected) && selectedDates == 1)
            {
        ...
        /// </summary>
        /// <param name="calendar">
        /// The BaseTool.Calendar associated with this automation peer.
        /// </param>
        ...
        {
            var  button = calendar.FindDayButtonFromDay(date);
            if (button != null)
            {
                var peer = FromElement(button);
                if (peer != null)
                {
        ...
        {
            // If WeekNumber functionality is supported by BaseTool.Calendar in the
